...
class Estimate {
    ...
    getEstimateByEstimateId(estimateId) {
        return new Promise((resolve, reject) => {
            pool.getConnection().then((conn) => {
                const sql = 'SELECT FUNC_REPAYMENT_AMOUNT_PER_MONTH(estimate.fixed_loan_amount, estimate.interest_rate, request.loan_period) as repayment_amount_per_month, estimate.estimate_id, estimate.interest_rate, estimate.interest_rate_type, estimate.repayment_type, estimate.overdue_interest_rate_1, estimate.overdue_interest_rate_2, estimate.overdue_interest_rate_3, estimate.overdue_time_1, estimate.overdue_time_2, estimate.overdue_time_3, estimate.early_repayment_fee, estimate.fixed_loan_amount, estimate.register_time, request.request_id, request.selected_estimate_id, request.loan_type, request.loan_amount, request.scheduled_time, request.overdue_record, request.loan_period, request.loan_reason, request.start_time, request.end_time, request.extra, request.job_type, request.status, request.region_1, request.region_2, request.region_3, request.apt_name, request.apt_kb_id, request.apt_price, request.apt_size_supply, request.apt_size_exclusive, agent.agent_id, agent.name, agent.photo, agent.greeting, agent.company_name as item_bank FROM estimate, agent, request WHERE 1=1 AND estimate.agent_id = agent.agent_id AND estimate.request_id = request.request_id AND estimate.estimate_id = ?';
